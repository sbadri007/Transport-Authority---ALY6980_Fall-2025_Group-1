...
class ExchangeAgent:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.stategraph_orchestrator = StateGraphOrchestrator()
    ...
    async def process_message(self, request: ChatRequest) -> ChatResponse:
        ...
        with span_context as span:
            ...
            try:
                ...
                if should_route:
                    # Step 3: Route to StateGraph Orchestrator
                    ...
                    agents_used = orchestrator_response.get('agents_called', [])
    ...
    async def _call_orchestrator(
        ...
        with tracer.start_as_current_span("call_stategraph_orchestrator"):
            try:
                # Use StateGraph instead of HTTP call
                result = await self.stategraph_orchestrator.process_message(
                    message, 
                    conversation_id
                )
                ...
                return {
                    ...
                    'execution_time': 0,  # Can add timing if needed
                    'intent': result.get('intent', ''),
                    'metadata': result.get('metadata', {})
                }

            except Exception as e:
                logger.error(f"StateGraph orchestrator call failed: {e}")
                raise
